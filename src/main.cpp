...
// flag width and height in bunnies
#define BUNNY_SIZE 1
#define NUM_GREEN_BUNNIES 24
#define NUM_YELLOW_BUNNIES 14
...
//   static float t = 0;
//
//   float radius = std::max(FLAG_WIDTH, FLAG_HEIGHT) * 10;
//   float x = cos(t * speed +
//                 (float)bunny_i * GREEN_BUNNY_PADDING / NUM_GREEN_BUNNIES) *
//             radius;
//   float y = sin(t * speed +
//                 (float)bunny_i * GREEN_BUNNY_PADDING / NUM_GREEN_BUNNIES) *
//             radius;
//   x = glm::clamp(x, -(float)FLAG_WIDTH / 2.0f, (float)FLAG_WIDTH / 2.0f);
//   y = glm::clamp(y, -(float)FLAG_HEIGHT / 2.0f, (float)FLAG_HEIGHT / 2.0f);
//
//   t += delta_t;
// }
...
int main(int argc, char *argv[]) {
  ...
  GLFWwindow *window;
  window = glfwCreateWindow(800, 600, "INF01047 - 587312 - Guilherme Guimarães",
                            NULL, NULL);
  if (!window) {
    glfwTerminate();
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    std::chrono::duration<float> elapsed = curr_time - last_time;
    /// clamping do delta time pra n explodir em caso de travas
    float delta_time = std::min(elapsed.count(), 0.1f);
    last_time = curr_time;
    stt.update(delta_time);
...
#define JADE_SURFACE 6
    ...
    model = Matrix_Translate(0.0f, -1.0f, 0.0f) *
            Matrix_Scale(FLAG_WIDTH * 2.0, 1.0f, FLAG_HEIGHT * 2.0);
    glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
...
void LoadShadersFromFiles() {
  ...
  g_surface_type_uniform = glGetUniformLocation(
      g_GpuProgramID,
      "surface_type"); // Material definido em shader_fragment.glsl
}
